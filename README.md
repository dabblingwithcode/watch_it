...
If you want to execute a function only on the first built (even in a StatelessWidget), you can use the `callOnce` function anywhere in your build function (similar to what `flutter_hooks` does). It has an optional `dispose` handler which will be called when the widget is disposed.
